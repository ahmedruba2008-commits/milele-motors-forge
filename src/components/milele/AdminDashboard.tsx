...
function Toggle({
  ...
  return (
    ...
    >
      <span
        className={`w-4 h-4 rounded-full border-2 ${active ? "bg-white border-white" : "border-white/40"}`}
      />
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <select
          className={inputCls}
          value={form.bodyType}
          onChange={(e) => setForm({ ...form, bodyType: e.target.value as FormState["bodyType"] })}
        >
        ...
        <div className="flex flex-wrap gap-2">
          {["Petrol", "Diesel", "Electric", "CNG"].map((f) => (
            <button
              type="button"
              key={f}
              className={pillCls(form.fuelType === f)}
              onClick={() => setForm({ ...form, fuelType: f as FormState["fuelType"] })}
            >
              {f}
            </button>
          ))}
        ...
        <div className="flex flex-wrap gap-2">
          {["Manual", "Automatic", "CVT", "DCT"].map((t) => (
            <button
              type="button"
              key={t}
              className={pillCls(form.transmission === t)}
              onClick={() => setForm({ ...form, transmission: t as FormState["transmission"] })}
            >
              {t}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <input
          ...
          type="number"
          step="0.01"
          placeholder="Price in Lakhs (e.g. 5.85)"
          value={form.price}
      ...
      <div className="flex flex-wrap gap-3">
        <Toggle
          active={form.isCertified}
          onChange={(v) => setForm({ ...form, isCertified: v })}
          label="✓ Certified Expert Inspected"
          colorClass="bg-brand-green"
        />
        <Toggle
          active={form.isHotDeal}
          onChange={(v) => setForm({ ...form, isHotDeal: v })}
          label="🔥 Hot Deal"
        ...
        {form.imageUrl ? (
          <div className="relative">
            ...
            <button
              ...
              className="absolute top-2 right-2 w-8 h-8 rounded-full bg-black/60 text-white grid place-items-center hover:bg-red-500"
            >
              <X size={16} />
            </button>
          </div>
          ...
          <button
            ...
            className="w-full border-2 border-dashed border-white/20 rounded-2xl p-8 text-center hover:border-brand-blue/60 transition-colors"
          >
      ...
      <div className="flex gap-3 pt-2">
        <button
          type="submit"
          className="flex-1 bg-brand-blue hover:bg-brand-blue-dark text-white font-bold uppercase tracking-widest text-sm py-4 rounded-xl shadow-lg shadow-blue-500/30 transition-all"
        >
        ...
        {editing && (
          <button
            ...
            className="px-6 py-4 rounded-xl text-white/70 hover:text-white bg-white/10 hover:bg-white/15 transition-all"
          >
            Cancel Edit
          </button>
        )}
  ...
  return (
    <div>
      <h3 className="font-syne font-bold text-2xl text-white flex items-center gap-3">
        Live Inventory{" "}
        <span className="text-brand-blue text-sm font-medium">({vehicles.length} vehicles)</span>
      </h3>
        ...
        {vehicles.length === 0 && (
          <div className="border-2 border-dashed border-white/15 rounded-2xl p-10 text-center">
            ...
            <p className="text-white/60 mt-3 text-sm">
              No vehicles in inventory. Add your first listing.
            </p>
          </div>
        )}
        {vehicles.map((v) => (
          <div
            key={v.id}
            className="bg-brand-slate rounded-xl p-4 flex items-center gap-4 border border-white/5"
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="text-white/50 text-sm truncate">
                ₹{v.price}L · {Number(v.mileage).toLocaleString("en-IN")}km · {v.city || "—"}
              </p>
              <div className="flex gap-1 mt-1">
                {v.isCertified && (
                  <span className="text-[10px] bg-brand-green/20 text-green-300 px-2 py-0.5 rounded-full">
                    Certified
                  </span>
                )}
                {v.isHotDeal && (
            ...
            {confirmId === v.id ? (
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => setConfirmId(null)} className="text-white/60 text-xs px-2">
                  Cancel
                </button>
              </div>
            ) : (
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="bg-red-500/20 hover:bg-red-500/40 text-red-400 px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1"
                >
...
function QuoteRequests() {
  ...
  const submit = (id: string) => {
    const d = drafts[id];
    if (!d?.price) {
      showToast("Enter a quote price", "error");
      return;
    }
  ...
  return (
    <div className="space-y-8">
      <div>
        <h3 className="font-syne font-bold text-2xl text-white">
          Pending Quotes <span className="text-brand-blue text-sm">({pending.length})</span>
        </h3>
          ...
          {pending.length === 0 && (
            <div className="border-2 border-dashed border-white/15 rounded-2xl p-10 text-center">
              ...
              <p className="text-white mt-3 font-medium">
                All caught up! No pending quote requests.
              </p>
            ...
            return (
              <div key={q.id} className="bg-brand-slate rounded-xl p-5 border border-brand-blue/20">
                <div className="flex justify-between items-start gap-2">
                  <span className="bg-brand-blue/20 text-brand-blue font-mono text-xs px-2 py-1 rounded">
                    #{ref}
                  </span>
                ...
                <div className="mt-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {[
                    ...
                    ["Mileage", q.vehicleMileage],
                    ["Fuel", q.vehicleFuelType],
                    ["Transmission", q.vehicleTransmission],
                    ["Condition", q.vehicleCondition],
                    ["City", q.vehicleCity],
                  ].map(([l, v]) => (
                    <div key={l}>
                ...
                <div className="mt-4">
                  <p className="text-white/60 text-sm mb-2">
                    Enter Your Official Quote Price (₹ Lakhs):
                  </p>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <input
                      value={d.price}
                      onChange={(e) => setDraft(q.id, "price", e.target.value)}
                      className="bg-brand-navy border border-white/20 text-white rounded-xl px-4 py-3 w-full sm:w-32 text-lg font-bold focus:border-brand-blue focus:outline-none"
                      placeholder="6.50"
                      type="number"
                      step="0.01"
                    />
                    <textarea
                      value={d.notes}
                      onChange={(e) => setDraft(q.id, "notes", e.target.value)}
                      className="bg-brand-navy border border-white/20 text-white rounded-xl px-4 py-3 flex-1 text-sm placeholder-white/30 focus:border-brand-blue focus:outline-none"
                      placeholder="Optional note (e.g. 'Subject to physical inspection')"
                      rows={2}
                    />
                  </div>
                  <button
                    onClick={() => submit(q.id)}
                    className="mt-3 bg-brand-green hover:bg-emerald-600 text-white font-bold px-6 py-3 rounded-xl transition-all"
                  >
                ...
                <div className="mt-3 flex gap-4 text-xs">
                  ...
                  <a
                    href={`https://wa.me/91${q.userPhone}?text=${encodeURIComponent(`Hi ${q.userName}, your Milele Motors quote is ready!`)}`}
                    target="_blank"
      ...
      <div>
        <h3 className="font-syne font-bold text-2xl text-white">
          Completed Quotes <span className="text-white/40 text-sm">({completed.length})</span>
        </h3>
              ...
              <div key={q.id} className="bg-white/5 rounded-xl p-4 border border-white/5">
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    <span className="bg-white/10 text-white/60 font-mono text-xs px-2 py-1 rounded">
                      #{ref}
                    </span>
                ...
                {open && (
                  <div className="mt-3 text-white/60 text-sm space-y-1">
                    <p>
                      Customer: <span className="text-white">{q.userName}</span> ·{" "}
                      <a href={`tel:${q.userPhone}`} className="text-brand-blue">
                        {q.userPhone}
                      </a>
                    </p>
                    <p>
                      Condition: {q.vehicleCondition} · {q.vehicleCity}
                    </p>
...
function LogoSettings() {
  ...
  useEffect(() => {
    setHasOverride(!!localStorage.getItem("milele_logo_override"));
  }, []);
    ...
    <div className="mt-10 border-t border-white/10 pt-8">
      ...
      <div className="mt-4 flex flex-col sm:flex-row items-start gap-6">
        ...
        <div className="flex-1 w-full">
          ...
          <button
            onClick={() => fileRef.current?.click()}
            className="border-2 border-dashed border-white/20 rounded-xl px-6 py-4 text-white/70 hover:border-brand-blue/60 transition-colors text-sm w-full sm:w-auto"
          >
            <Upload size={16} className="inline mr-2" /> Update Logo
          </button>
          {hasOverride && (
            <button onClick={reset} className="ml-3 text-brand-blue text-sm hover:underline">
              Reset to Default
            </button>
          )}
          <p className="text-white/40 text-xs mt-3">
            Uploading here overrides the default logo for this session. For a permanent change,
            replace <code className="text-white/60">src/assets/logo.png</code>.
          </p>
        </div>
      </div>
    </div>
      ...
      <div className="sticky top-0 z-10 bg-brand-navy/95 backdrop-blur border-b border-white/10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Logo size="sm" />
            <span className="bg-brand-blue text-white text-xs px-3 py-1 rounded-full font-bold">
              Admin Panel
            </span>
            {userEmail && (
              <span className="hidden sm:inline text-white/50 text-xs">{userEmail}</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            <a
              href="/"
              className="bg-white/10 hover:bg-white/20 text-white px-3 py-2 rounded-lg text-xs font-bold"
            >
              View Site
            </a>
            <button
              onClick={onSignOut}
              className="bg-white/10 hover:bg-white/20 text-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-2"
            >
              <X size={14} /> Sign Out
            </button>
          </div>
